...
n_folds <- 10
eps <- ifelse(nrow(learn)>ncol(learn), 0.0001, 0.01) # see ?glmnet -> eps = 'lambda.min.ratio'
...
for (i in 1:n_alpha_tune){
  ...
  set.seed(5959)
  # set.seed(9595)
  model.list[[i]] <- glmnet::cv.glmnet(learn[, -1], learn[, 1], family = "binomial", alpha = alpha_tune[i], lambda = lambda.seq, type.measure = "class", nfolds = n_folds)
...
within1SE_selection <- which(val.perform >= val.perform[best.val.mod] - val.perform.sd[best.val.mod])

preds <- list()
for (i in 1:length(model.list)) {
  these_models <- model.list[[i]]
  preds[[i]] <- predict(these_models, test[, -1], s = these_models$lambda, type = "class") %>% unname
}
...
best.eval.mod <- within1SE_selection[which.max(eval_perform[within1SE_selection])]
...
rank_mat <- data.frame(model = within1SE_selection, val.rank = val.ranks, eval.rank = eval.ranks)
...
results_mat[order(results_mat[,2] , decreasing=TRUE),]
...
eval_boot <- boot::boot(within1SE_similar_mat, function(x, i) colMeans(x[i, ]), R = 10000)
alpha <- 0.05
measure <- "class"
...
mabt_ci <- MabtCi(eval_boot, alpha, measure, select_id, within1SE_preds, cls_thresh, labs)

# standard cis

n_test <- nrow(preds)
n_success <- eval_perform[best.val.mod] * n_test
default_cis <- DescTools::BinomCI(n_success, n_test, conf.level = 1-alpha, sides = "left", method = c("wilson", "clopper-pearson", "wald"))
