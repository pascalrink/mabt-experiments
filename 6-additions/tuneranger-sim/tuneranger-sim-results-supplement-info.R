...
# This scripts produces the random forest simulation plots in the supplement
# document. 
...
ggplot(df_covering, aes(x = factor(n_eval), y = ci.lwr, fill = ci.method, pattern = ci.method)) + 
  ...
  guides(pattern = guide_legend(override.aes = list(fill = my_colors)),
         fill = "none")
# .myggsave("6-additions/tuneranger-sim/figures/tr-sim-lwr-bound.eps", last_plot())
...
ggplot(df_covering, aes(x = factor(n_eval), y = gt-ci.lwr, fill = ci.method, pattern = ci.method)) + 
  ...
  guides(pattern = guide_legend(override.aes = list(fill = my_colors)),
         fill = "none")
# .myggsave("6-additions/tuneranger-sim/figures/tr-sim-tightness.eps", last_plot())
...
subset(df, ci.method %in% c("Wilson", "MABT")) %>% 
  ...
  guides(pattern = guide_legend(override.aes = list(fill = my_colors[c(3, 2)])),
         fill = "none")
# .myggsave("6-additions/tuneranger-sim/figures/tr-sim-true-perform.eps", last_plot())
