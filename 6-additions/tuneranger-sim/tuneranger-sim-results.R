...
df$ci.method <- recode(df$ci.method, 
                       "clopper-pearson_sidak" = "CP+S", "clopper-pearson" = "CP", 
                       "wald_sidak" = "Wald+S", "mabt" = "MABT", 
                       "wilson_sidak" = "Wilson+Sidak", "wald" = "Wald", 
                       "wilson" = "Wilson")
...
# add 'undercovers y/n'
coverage_table <- aggregate(covers ~ ci.method + n_eval, df, mean)
...
coverage_table %>% ggplot(aes(x = factor(n_eval), y = coverage, fill = ci.method, pattern = ci.method)) + 
  ...
  coord_cartesian(ylim = c(0.92, 0.965)) + 
  ...
  guides(pattern = guide_legend(override.aes = list(fill = my_colors)),
         fill = "none")
# .myggsave("6-additions/tuneranger-sim/figures/fig-14.eps", last_plot())
...
ggplot(df_covering, aes(x = factor(n_eval), y = ci.lwr, fill = ci.method, pattern = ci.method)) + 
  ...
  guides(pattern = guide_legend(override.aes = list(fill = my_colors)),
         fill = "none")
# .myggsave("6-additions/tuneranger-sim/figures/fig-15.eps", last_plot())
...
ggplot(df_covering, aes(x = factor(n_eval), y = gt-ci.lwr, fill = ci.method, pattern = ci.method)) + 
  ...
  guides(pattern = guide_legend(override.aes = list(fill = my_colors)),
         fill = "none")
# .myggsave("6-additions/tuneranger-sim/figures/fig-16.eps", last_plot())
...
subset(df, ci.method %in% c("Wilson", "MABT")) %>% 
  ...
  guides(pattern = guide_legend(override.aes = list(fill = my_colors[c(3, 2)])),
         fill = "none")
# .myggsave("6-additions/tuneranger-sim/figures/fig-17.eps", last_plot())
